...
from django.db.models import CharField, Value, Prefetch, Count
...
def Myposts(request):
    ...
    try:
        ...
        profilePhoto = None
    return render(request, "postapp/mypost.html", {'posts':posts , "profilePhoto":profilePhoto})
...
def Update(request, id):
    ...
    if request.method == "POST":
        # Passer l'instance existante au formulaire
        form = Ticketform(request.POST, request.FILES or None, instance=ticket)
        
        if form.is_valid():
            # Garder l'image existante si aucune nouvelle n'est fournie
...
def Deletecomment(request,id):
    ...
    if delt.user != request.user:  # Vérification supplémentaire
        return redirect('postapp:mydash_page')
    if request.method=="POST":
        delt.delete()
        return redirect('postapp:mydash_page')
...
def Review_ticket(request, id):
    ...
    if request.method == "POST":
        ...
        return render(request, "postapp/revticket.html", {"ticket_to_review":ticket_to_review, 
                                    "reviewform":reviewform, "profilePhoto":profilePhoto,
                                    "error": "Le commentaire ne peut pas être vide" if request.method == "POST" else None})
 """
@login_required
def Review_ticket(request, id):
    try:
    ...
    if request.method == "POST":
        ...
        if reviewform.is_valid():
            ...
            return redirect("postapp:mydash_page")
        # Si le formulaire est invalide, réafficher avec les erreurs
        return render(request, "postapp/revticket.html", {
